...
One major source of this difference is shown at the quantity of calls to cfi, O0 has 6 calls to cfi in total, while O1 has a dramatic increase to 22 calls, and O3 with a small increase compared to O1 with only 24 calls.
Although it seems to be the largest difference it of course cannot be the ONLY reason. Another difference is that O1/O3 became less jumpy for if and while statements. This can be exemplified best at line 31 on projO0 compared with line 45 on O1, and line 46 on O3, where after comparing O0 will jump to continue to run while O1/O3 only jumps to end it.
...
One final difference between O0 and O1 is the use of ".cfi_restore_state" which only appears in O3.
